...
- **`window`**: persisted size (logical pixels) of the menu-bar popover (the `main` window). The popover is undecorated/transparent and `resizable: true`; the user drags invisible edge/corner grips (`ResizeGrips` in the frontend, forwarding to Tauri's `startResizeDragging`). The backend restores this size in `setup()` *before* the first show (so there's no resize flash) and saves the current size in the blur handler when the popover hides — a low-churn save point versus writing on every drag frame. A restored size is clamped to the `minWidth`/`minHeight` in `tauri.conf.json` (mirrored as `MIN_POPOVER_*` constants in `main.rs`). Absent `window` means "use the `tauri.conf.json` default size." Window **position** is not persisted — the popover is always re-anchored under the tray icon on each show, via the positioner's `move_window_constrained(TrayCenter)` which clamps it to the monitor bounds so it stays fully on-screen (never cropped when the tray icon sits near the far-right edge or the popover has been resized wider).
- **`theme`**: chosen UI appearance — `"light"`, `"dark"`, or `"system"` (issue #12). Absent means `"system"`. Set via the **Appearance** tab in the Settings window (`app_settings_set_theme`), read on launch by every window (`app_settings_get_theme`). Each webview applies the theme to its own `<html>`: an explicit choice sets `data-theme="light|dark"`, while `"system"` removes the attribute so the `prefers-color-scheme` media query in `styles.css` follows the macOS appearance live. `app_settings_set_theme` broadcasts a `theme-changed` event so the popover, Settings, and Logs windows re-apply without a restart (see `frontend/src/theme.ts`). The blur handler that saves `window` load-merges, so persisting one field never clobbers the other.
