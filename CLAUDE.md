...
- **Editor autocomplete.** At startup the app mirrors the embedded schema to `~/.maiestro/schemas/repo-settings.schema.json` (`write_schema_file`), so a hand-editor can point a `$schema` key at it for autocomplete in VS Code.
