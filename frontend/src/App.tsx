...
// One-time onboarding (issue #98). A branded webview dialog — rather than a native
// alert, which can only show the generic OS icon — so it shows the mAIestro logo
// and name and can grow more options over time. The backend opens this window only
// on the very first run; pressing "Get started" (or closing the window, which
// accepts the defaults) records the choices and marks onboarding complete.
//
// To add an option: add a piece of state and an `OnboardingOption` row, then
// thread its value into `completeOnboarding` (and the backend command).
function Onboarding() {
